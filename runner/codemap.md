...
- **One persistent cache, outside the boot cycle:** the `runner-cache` named volume is mounted at `/home/runner/.cache` and survives recreations and rebuilds. pip/uv cache there by default; compose env vars (`npm_config_cache`, `GOMODCACHE`, `GOCACHE`, `GRADLE_USER_HOME`, `CARGO_HOME`, `RUSTUP_HOME`) redirect the other toolchains into it, and `RUNNER_TOOL_CACHE` keeps setup-* runtimes there too. Rust is not baked into the image: workflows install it per-run and the install persists on the volume. The image seeds the mount point with runner ownership so a fresh volume just works. The entrypoint does no cache work; cleaning is manual (`docker compose exec runner sh -c 'rm -rf /home/runner/.cache/*'`).
