...
- **One persistent cache, outside the boot cycle:** `./runner-cache` is bind-mounted at `/home/runner/.cache` and survives recreations and rebuilds. pip/uv cache there by default; compose env vars (`npm_config_cache`, `GOMODCACHE`, `GOCACHE`, `GRADLE_USER_HOME`, `CARGO_HOME`) redirect the other toolchains into it. The entrypoint does no cache work; cleaning is manual (`rm -rf runner-cache/*`).
...
- Orchestrated by: `docker-compose.yml` `runner` service (`restart: always`, `cap_drop: ALL`, no-new-privileges, tmpfs scratch, cache bind mount `./runner-cache` → `/home/runner/.cache` + toolchain cache env vars).
